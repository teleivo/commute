...
Start a 3-node cluster. Each node listens on its own host port; gossip flows between containers
on port 8080:
...
Increment on node-0, sleep for gossip, then read from node-1:
...
# node-1 reads, captures the context, then node-2 re-adds.
...
sleep 6
curl localhost:8080/sets/fruits   # apple is still in the set
```

...
// triples (i, n, l): replica id, counter, and a list of siblings whose dots are implicit from the
	...
	for id := range ids {
		if entry, ok := ds.state[id]; ok {
			if otherEntry, ok := other.state[id]; ok {
				var result dvvEntry[T]
				if entry.counter >= otherEntry.counter {
					result = dvvEntry[T]{
						counter: entry.counter,
						values:  entry.values[0:min(int(entry.counter-otherEntry.counter)+len(otherEntry.values), len(entry.values))],
					}
				} else {
					result = dvvEntry[T]{
						counter: otherEntry.counter,
						values:  slices.Clone(otherEntry.values[0:min(int(otherEntry.counter-entry.counter)+len(entry.values), len(otherEntry.values))]),
					}
				}
				ds.state[id] = result
			}
		} else {
			ds.state[id] = dvvEntry[T]{
				counter: other.state[id].counter,
				values:  slices.Clone(other.state[id].values),
			}
		}
	}
}
...
func (ds *DVVSet[T]) UnmarshalJSON(data []byte) error {
	...
	if v.State == nil {
		v.State = make(map[NodeID]dvvEntry[T])
	}
	ds.state = v.State
	return nil
}

...
func TestDVVSetUnmarshalNullState(t *testing.T) {
	// A wire payload with "state": null must unmarshal into a usable DVVSet: subsequent writes
	// should not panic with "assignment to entry in nil map".
	d := &DVVSet[string]{}
	err := json.Unmarshal([]byte(`{"state":null}`), d)

...
// TestNotifyDeadRemovesHTTPPeer verifies that when SWIM declares a peer dead, gossip to that peer
// stops. Notify receives a SWIM UDP address; the server matches it to the HTTP peer by hostname.
func TestNotifyDeadRemovesHTTPPeer(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		c := newCluster(t, 3)
		c.startGossip(t.Context())
